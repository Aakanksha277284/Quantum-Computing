...
class ControlledU:
    ...
    def to_unitary(self) -> np.ndarray:
        ...
        N = 2**self.n
        U = np.eye(N, dtype= np.complex128)
        target_bit = 1 << self.target

        state0 = (1 << self.n) - 1
        state0 &= ~target_bit
        state1 = state0 | target_bit

        U[state0, state0] = self.unitary[0,0]
        U[state0, state1] = self.unitary[0,1]
        U[state1, state0] = self.unitary[1,0]
        U[state1, state1] = self.unitary[1,1]
...
class CU:
    ...
    def to_unitary(self) -> np.ndarray:
        ...
        target_bit = 1 << self.target
        control_bit = 1 << self.control

        for i in range(N):
            if ((i & target_bit) == 0 and ((i & control_bit) !=0)):
                j = i | target_bit
...
class CNOT:
    ...
    def to_unitary(self) -> np.ndarray:
        ...
        gate = CU(n= self.n, control= self.control, target= self.target, unitary= X)
        
        return gate.to_unitary()
        # TODO: implement.
...
def circuit_to_unitary(circuit: Circuit) -> np.ndarray:
    ...
    """
    result = np.eye(circuit[0].to_unitary().shape[0], dtype = np.complex128)

    for gate in circuit:
        result = gate.to_unitary() @ result
...
def decompose_vector(vec: np.ndarray) -> TwoLevels:
    ...
    for i in range(n-1,0,-1):
        ...
        if not np.isclose(norm, 0):
            block = align(x,y,norm)
            result.append(TwoLevel(
                size = n,
                level0= i- 1,
                level1= i,
                unitary= block
        ...
        current[i-1] = norm
        current[i] = 0
       # print("decompose_vector returned", len(result))
    return result
    # TODO: implement.
...
def expand_twolevels(input: TwoLevels, n: int) -> TwoLevels:
    ...
    for tl in input:
        ...
        output.append(TwoLevel(
            size = n,
            level0= tl.level0 + shift,
            level1= tl.level1 + shift,
            unitary= tl.unitary
        ))
...
def decompose_unitary(u: np.ndarray) -> TwoLevels:
    ...
    for col in range(N - 1):
        ...
        for tl in tls:
            current = tl.to_unitary() @ current

    phase = np.angle(current[-1,-1])
    phase_gate = TwoLevel(
        size = N,
        level0= N - 2,
        level1= N - 1,
...
def abc_reconstruct(d: ABC) -> np.ndarray:
    ...
    return(
        np.exp(1j*d.alpha) * d.A @ X @ d.B @ X @ d.C 
    )
    # TODO: implement.
...
def controlled_circuit(
    ...
    for q in range(n):
        if q == target:
            continue
        if not control_vals[q]: 
            circuit.append(
            SingleQubitGate(n= n, qubit= q, unitary= X)
        )

    circuit.append(
        ControlledU(n=n, target=target, unitary=unitary)
    )
...
def decompose_controlled(
    ...
    if k == 1:
        if np.allclose(u,X):
            return[
                CNOT(
                    n=n, control=controls[0], target=target
                )
            ]
        return[
            CU(
                n=n, control=controls[0], target=target, unitary=u
            )
        ]
...
def decompose_controlledU(g: ControlledU) -> Circuit:
    ...
    return decompose_controlled(
        g.n, controls, g.target, g.unitary
    )
    # TODO: implement.
...
def decompose_cu(g: CU) -> Circuit:
    ...
    return[
        SingleQubitGate(g.n, g.control, phase_gate),
        SingleQubitGate(g.n, g.target, d.C),
        CNOT(g.n, g.control, g.target),
        SingleQubitGate(g.n, g.target, d.B),
        CNOT(g.n, g.control, g.target),
        SingleQubitGate(g.n, g.target, d.A)
    ]
...
def ht_gates(n: int, qubit: int, word: str) -> Circuit:
    ...
    for gate in reversed(word):
        if gate == "H":
            circuit.append(
                SingleQubitGate(n, qubit, rotation.H)
            )
        else:
            circuit.append(
                SingleQubitGate(n, qubit, T)
            )
